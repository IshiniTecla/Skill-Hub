...
const Profile = () => {
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      const updateData = {
        ...
        bio: editForm.bio,
        // Don't send email since it's not editable
      };

      // Use UserService for the update operation
      ...
      if (updatedUserData) {
        ...
        const updatedUser = new UserModel(updatedUserData);
        setProfile(updatedUser);
        setIsEditModalOpen(false);

        // Update the user in localStorage to maintain session consistency
  ...
  const handleFollow = async () => {
    ...
    try {
      ...
      fetchFollowers(profile.id).catch((err) =>
        console.warn("Failed to refresh followers:", err)
      );
  ...
  const handleUnfollow = async () => {
    ...
    try {
      ...
      updatedProfile.followersCount = Math.max(
        (updatedProfile.followersCount || 0) - 1,
        0
      );
      setProfile(updatedProfile);
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      // Use UserService for the delete operation with proper user ID
  ...
  const handleSavePost = async (postId) => {
    try {
      ...
      fetchSavedPosts().catch((err) =>
        console.warn("Failed to refresh saved posts:", err)
      );
    } catch (err) {
  ...
  const handleUnsavePost = async (postId) => {
    try {
      ...
      toast.error(err.message || "Failed to remove post from saved items");
    }
  };
          ...
          <div className="profile-username-container">
            ...
            {isOwnProfile ? (
              ...
              <div className="profile-actions">
                {currentUser ? (
                  isFollowing ? (
                    <button className="following-btn" onClick={handleUnfollow}>
                      Following
                    </button>
                  ) : (
          ...
          posts.length > 0 ? (
            posts.map((post) => (
              <div className="post-item" key={post.id}>
                <img
                  src={
                    post.imageUrl || getImageUrl(`/api/posts/${post.id}/image`)
                  }
                  alt="Post"
                  onError={(e) => {
                    e.target.onerror = null;
        ...
        <div className="modal-overlay">
          <div className="delete-modal">
            ...
            <div className="modal-content">
              ...
              <p>
                All your posts, comments, and account data will be permanently
                removed.
              </p>
            </div>
                      ...
                      {follower.profileImage ? (
                        ...
                        <div className="user-initials">
                          {(
                            ...
                            (follower.lastName ? follower.lastName[0] : "")
                          ).toUpperCase() ||
